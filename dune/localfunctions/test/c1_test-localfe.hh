...
#ifndef DUNE_LOCALFUNCTIONS_TEST_C1_TEST_LOCALFE_HH
#define DUNE_LOCALFUNCTIONS_TEST_C1_TEST_LOCALFE_HH

/** \file \brief Unit tests for LocalFiniteElement objects that utilize derivative degrees of
 * freedom
 *
 *  \note This file is an adapted copy from dune-localfunctions
 *
...
class ShapeFunctionAsFunction
{
  public:
    ...
    friend ShapeFunctionDerivativeAsCallable<FE> derivative(ShapeFunctionAsFunction const &t)
    {
      return ShapeFunctionDerivativeAsCallable(t.fe_, t.shapeFunction_);
    }
...
public:
  ShapeFunctionAsCallable(const FE &fe, int shapeFunction) : fe_(fe), shapeFunction_(shapeFunction)
  {
  }
  ...
  friend ShapeFunctionDerivativeAsCallable<FE> derivative(ShapeFunctionAsCallable const &t)
  {
    return ShapeFunctionDerivativeAsCallable(t.fe_, t.shapeFunction_);
  }
...
public:
  ShapeFunctionDerivativeAsCallable(const LFE &fe, int shapeFunction)
      : fe_(fe), shapeFunction_(shapeFunction)
  {
  }
  ...
  auto friend derivative(ShapeFunctionDerivativeAsCallable const &t)
  {
    ...
    {
      Dune::FieldVector<Dune::FieldVector<ValueType, dim>, dim> hessian;
      ...
      for (std::size_t i = 0; i < dim; ++i)
        for (std::size_t j = 0; j < dim; ++j)
        {
          ...
          hessian[i][j] = partials[t.shapeFunction_];
        }
      return hessian;
    };
  }
...
template<class FE>
auto getDualityMatrix(const FE& fe)
{
  Dune::FieldMatrix<typename FE::Traits::LocalBasisType::Traits::RangeFieldType, FE::size(), FE::size()> ret;
  ...
  for (size_t i = 0; i < fe.size(); ++i)
  {
    ...
    // The i-th shape function as a function that 'interpolate' can deal with
    ShapeFunctionAsCallable<FE> f(fe, i);
...
public:
  ...
  // TODO replace those DerivativeTraits by something and remove dependency to derivative.hh
 ...
 *   not differentiable.
 */
template<unsigned int diffOrder = 0, class FE>
            ...
            const std::function<bool(const typename FE::Traits::LocalBasisType::Traits::DomainType&)> derivativePointSkip = nullptr)
{
  ...
  else
  {
    ...
    if (diffOrder != 0) {
      success = false;
      std::cout << "invalid Test! checking for higher partials but not for jacobian\n";
    }
  }
...
#define TEST_FE3(A,B,C) { bool b = testFE<C>(A, B); std::cout << "testFE(" #A ", " #B ", " #C ") " << (b?"succeeded\n":"failed\n"); success &= b; }
#define TEST_FE4(A,B,C,D) { bool b = testFE<C>(A, B, D); std::cout << "testFE(" #A ", " #B ", " #C ", " #D ") " << (b?"succeeded\n":"failed\n"); success &= b; }
