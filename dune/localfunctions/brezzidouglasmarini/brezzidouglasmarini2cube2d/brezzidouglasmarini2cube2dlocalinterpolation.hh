  ...
  public:
    ...
    void interpolate(const F& f, std::vector<C>& out) const
    {
           ...
           it != rule.end(); ++it)
      {
        ...
        out[2] += (y[0]*n0[0] + y[1]*n0[1])*weight_2*sign0;

        localPos[0] = 1.0;
        localPos[1] = qPos;
        y = f(localPos);
        out[3] += (y[0]*n1[0]+y[1]*n1[1])*weight_0*sign1;
        out[4] += (y[0]*n1[0]+y[1]*n1[1])*(-weight_1);
        out[5] += (y[0]*n1[0]+y[1]*n1[1])*weight_2*sign1;

        localPos[0] = qPos;
        localPos[1] = 0.0;
        y = f(localPos);
        out[6] += (y[0]*n2[0] + y[1]*n2[1])*weight_0*sign2;
        out[7] += (y[0]*n2[0] + y[1]*n2[1])*(-weight_1);
        out[8] += (y[0]*n2[0] + y[1]*n2[1])*weight_2*sign2;

        localPos[0] = qPos;
        localPos[1] = 1.0;
        y = f(localPos);
        out[9] += (y[0]*n3[0] + y[1]*n3[1])*weight_0*sign3;
        out[10] += (y[0]*n3[0] + y[1]*n3[1])*weight_1;
        out[11] += (y[0]*n3[0] + y[1]*n3[1])*weight_2*sign3;
      }
