  ...
  public:
    ...
    void interpolate (const F& f, std::vector<C>& out) const
    {
      ...
      for (typename Dune::QuadratureRule<Scalar,1>::const_iterator it=rule.begin(); it!=rule.end(); ++it)
      {
        ...
        out[3] += (y[0]*n0[0] + y[1]*n0[1])*weight_1/c0;

        localPos[0] = 0.0;
        localPos[1] = qPos;
        y = f(localPos);
        out[1] += (y[0]*n1[0] + y[1]*n1[1])*weight_0*sign1/c1;
        out[4] += (y[0]*n1[0] + y[1]*n1[1])*(-weight_1)/c1;

        localPos[0] = 1.0 - qPos;
        localPos[1] = qPos;
        y = f(localPos);
        out[2] += (y[0]*n2[0] + y[1]*n2[1])*weight_0*sign2/c2;
        out[5] += (y[0]*n2[0] + y[1]*n2[1])*weight_1/c2;
      }
    }
