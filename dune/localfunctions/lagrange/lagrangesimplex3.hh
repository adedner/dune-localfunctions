...
#ifndef DUNE_LOCALFUNCTIONS_LAGRANGE_LAGRANGESIMPLEX3_HH
#define DUNE_LOCALFUNCTIONS_LAGRANGE_LAGRANGESIMPLEX3_HH
...
namespace Dune { namespace Impl
{
  ...
  struct LagrangeSimplexTraits3
  {
    static constexpr bool is_static_order = (k>=0);

    constexpr LagrangeSimplexTraits3(int /*order*/ = k)
    {
  ...
  struct LagrangeSimplexTraits3<dim,-1>
  {
    const int k_;
    ...
    constexpr explicit LagrangeSimplexTraits3(int order)
      : k_(order)
  ...
  struct LagrangeSimplexLocalBasisBuffers3
  {
  ...
  struct LagrangeSimplexLocalBasisBuffers3<R,dim,-1>
  {
    ...
    auto makeValueBuffer(int order) const
    {
      ...
      return Std::mdspan<R,E>{cache_.data(), E(order+1)};
    }
  ...
  class LagrangeSimplexLocalBasis3
    : public LagrangeSimplexTraits3<dim,polynomialOrder>
    , private LagrangeSimplexLocalBasisBuffers3<R,dim,polynomialOrder>
  {
    template <class> friend class LagrangeSimplexLocalInterpolation3;

    using Base = LagrangeSimplexTraits3<dim,polynomialOrder>;
    using Buffers = LagrangeSimplexLocalBasisBuffers3<R,dim,polynomialOrder>;
    static constexpr bool is_static_order = Base::is_static_order;

public:
    constexpr LagrangeSimplexLocalBasis3(int k = polynomialOrder)
      : Base(k)
    {}
...
private:
  ...
  public:
                          ...
                          std::vector<typename Traits::RangeType>& out) const
    {
      ...
      if constexpr(not is_static_order) {
        switch (k) {
          case 2: return LagrangeSimplexLocalBasis3<D,R,dim,2>{}.evaluateFunction(x,out);
          case 3: return LagrangeSimplexLocalBasis3<D,R,dim,3>{}.evaluateFunction(x,out);
          case 4: return LagrangeSimplexLocalBasis3<D,R,dim,4>{}.evaluateFunction(x,out);
          case 5: return LagrangeSimplexLocalBasis3<D,R,dim,5>{}.evaluateFunction(x,out);
          case 6: return LagrangeSimplexLocalBasis3<D,R,dim,6>{}.evaluateFunction(x,out);
          case 7: return LagrangeSimplexLocalBasis3<D,R,dim,7>{}.evaluateFunction(x,out);
          case 8: return LagrangeSimplexLocalBasis3<D,R,dim,8>{}.evaluateFunction(x,out);
          default: /* continue */;
        }
      }
      ...
      DUNE_THROW(NotImplemented, "LagrangeSimplexLocalBasis3 for k>=2 only implemented for dim<=3");
    }
                          ...
                          std::vector<typename Traits::JacobianType>& out) const
    {
      ...
      if constexpr(not is_static_order) {
        switch (k) {
          case 2: return LagrangeSimplexLocalBasis3<D,R,dim,2>{}.evaluateJacobian(x,out);
          case 3: return LagrangeSimplexLocalBasis3<D,R,dim,3>{}.evaluateJacobian(x,out);
          case 4: return LagrangeSimplexLocalBasis3<D,R,dim,4>{}.evaluateJacobian(x,out);
          case 5: return LagrangeSimplexLocalBasis3<D,R,dim,5>{}.evaluateJacobian(x,out);
          case 6: return LagrangeSimplexLocalBasis3<D,R,dim,6>{}.evaluateJacobian(x,out);
          case 7: return LagrangeSimplexLocalBasis3<D,R,dim,7>{}.evaluateJacobian(x,out);
          case 8: return LagrangeSimplexLocalBasis3<D,R,dim,8>{}.evaluateJacobian(x,out);
          default: /* continue */;
        }
      }
  ...
  class LagrangeSimplexLocalCoefficients3
    : public LagrangeSimplexTraits3<dim,polynomialOrder>
  {
    using Base = LagrangeSimplexTraits3<dim,polynomialOrder>;
  ...
  public:
    //! \brief Constructor
    LagrangeSimplexLocalCoefficients3 (int order = polynomialOrder)
      : Base(order)
      , localKeys_(Base::size())
    {
      ...
      DUNE_THROW(NotImplemented, "LagrangeSimplexLocalCoefficients3 only implemented for k<=1 or dim<=3!");
    }
     ...
     */
    constexpr LagrangeSimplexLocalCoefficients3 (const std::array<unsigned int, dim+1> vertexMap)
      : localKeys_(Base::size())
    {
      if (dim!=2 && dim!=3)
        DUNE_THROW(NotImplemented, "LagrangeSimplexLocalCoefficients3 only implemented for dim==2 and dim==3!");
    ...
    constexpr LagrangeSimplexLocalCoefficients3(const VertexMap &vertexmap)
      : localKeys_(Base::size())
    {
      if (dim!=2 && dim!=3)
        DUNE_THROW(NotImplemented, "LagrangeSimplexLocalCoefficients3 only implemented for dim==2 and dim==3!");
  ...
  private:
    ...
    constexpr void generateLocalKeys(const std::array<unsigned int, dim+1> vertexMap)
    {
      ...
      if (dim!=3)
        DUNE_THROW(NotImplemented, "LagrangeSimplexLocalCoefficients3 only implemented for dim==3!");
  ...
  class LagrangeSimplexLocalInterpolation3
    : public LocalBasis::Base
  {
    using Base = typename LocalBasis::Base;

  public:
    constexpr LagrangeSimplexLocalInterpolation3 () = default;

    //! \brief Constructor storing a reference to the local basis
    explicit constexpr LagrangeSimplexLocalInterpolation3 (const LocalBasis& localBasis)
      : Base(localBasis)
    {}
    ...
    constexpr void interpolate (const F& f, std::vector<C>& out) const
    {
      ...
      if (dim!=3)
        DUNE_THROW(NotImplemented, "LagrangeSimplexLocalInterpolation3 only implemented for dim<=3!");
...
namespace Dune
{
   ...
   * \tparam polynomialOrder polynomial order (or -1 for dynamic order)
   *
  ...
  class LagrangeSimplexLocalFiniteElement3
  {
  public:
    /** \brief Export number types, dimensions, etc.
     */
    using Traits = LocalFiniteElementTraits<
      Impl::LagrangeSimplexLocalBasis3<D,R,d,polynomialOrder>,
      Impl::LagrangeSimplexLocalCoefficients3<d,polynomialOrder>,
      Impl::LagrangeSimplexLocalInterpolation3<Impl::LagrangeSimplexLocalBasis3<D,R,d,polynomialOrder> > >;

    //! \brief Constructor for static order
    constexpr LagrangeSimplexLocalFiniteElement3()
      : basis_()
      , coefficients_()
    ...
    explicit constexpr LagrangeSimplexLocalFiniteElement3(int k)
      : basis_(k)
      , coefficients_(k)
      , interpolation_(basis_)
    {
      if (k < 0)
        DUNE_THROW(Dune::InvalidStateException, "LagrangeSimplexLocalFiniteElement3: Order k must be non-negative!");
    }

    /** \brief Constructs a finite element given a vertex reordering
     * */
    template<typename VertexMap>
    explicit constexpr LagrangeSimplexLocalFiniteElement3(const VertexMap& vertexmap) requires(polynomialOrder >= 0)
      : basis_()
...
#endif   // DUNE_LOCALFUNCTIONS_LAGRANGE_LAGRANGESIMPLEX3_HH
